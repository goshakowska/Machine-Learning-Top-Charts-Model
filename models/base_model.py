...
from src.data.make_dataset import prepare_dataset, RAW_DATA_DIR, PROCESSED_DATA_DIR
import os
...
class BaseModel(AbstractModel):
    ...
    def predict(self, X: DataFrame) -> dict:
        """
        ...
        """
        self.validate_provided_data(X)
        X = self.process_data(X)
        top_tracks_df = prepare_dataset(X)
        top_tracks_df = top_tracks_df.sort_values(by="play_count", ascending=False).head(self.parameters["tracks_per_list"])
...
def predict_with_base_model(tracks_per_list=None, music_genre=None) -> DataFrame:
    ...
    session_df = base_model.load_data(session_file)
